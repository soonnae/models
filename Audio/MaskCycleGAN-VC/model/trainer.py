...
class MaskCycleGANVCTrainer(object):
    ...
    def __init__(self, args):
        ...
        self.validation_A_dir = os.path.join(args.origin_data_dir, args.speaker_A_id)
        self.output_A_dir = os.path.join(args.output_data_dir, args.speaker_A_id)
        self.validation_B_dir = os.path.join(args.origin_data_dir, args.speaker_B_id)
        self.output_B_dir = os.path.join(args.output_data_dir, args.speaker_B_id)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.num_epochs + 1):
            ...
            for i, (real_A, mask_A, real_B, mask_B) in enumerate(self.train_dataloader):
                ...
                identityLoss = flow.mean(flow.abs(real_A - identity_A)) + flow.mean(
                ...
                cycled_B = self.generator_A2B(generated_A, flow.ones_like(generated_A))
                ...
                cycled_A = self.generator_B2A(generated_B, flow.ones_like(generated_B))
    ...
    def infer(self):
        ...
        for file in os.listdir(infer_A_dir):
            ...
            coded_sp_norm = (
                coded_sp_transposed - self.dataset_A_mean
            ) / self.dataset_A_std
            ...
            coded_sp_converted = (
                coded_sp_converted_norm * self.dataset_B_std + self.dataset_B_mean
            )
            coded_sp_converted = coded_sp_converted.T
            coded_sp_converted = np.ascontiguousarray(coded_sp_converted).astype(
                np.double
            )
    ...
    def validation_for_B_dir(self):
        ...
        for file in os.listdir(validation_B_dir):
            ...
            coded_sp_transposed = coded_sp.T
            coded_sp_norm = (
                coded_sp_transposed - self.dataset_B_mean
            ) / self.dataset_B_std
            coded_sp_norm = np.array([coded_sp_norm])
            ...
            coded_sp_converted_norm = np.squeeze(coded_sp_converted_norm)
            coded_sp_converted = (
                coded_sp_converted_norm * self.dataset_A_std + self.dataset_A_mean
            )
            coded_sp_converted = coded_sp_converted.T
            coded_sp_converted = np.ascontiguousarray(coded_sp_converted).astype(
                np.double
            ...
            sf.write(
                os.path.join(output_B_dir, "convert_" + os.path.basename(file)),
                wav_transformed,
                sampling_rate,
            )
