...
EASYNLP_REMOTE_ROOT = (
    "https://atp-modelzoo-sh.oss-cn-shanghai.aliyuncs.com/release/easynlp"
)
EASYNLP_REMOTE_MODELZOO = (
    "http://atp-modelzoo-sh.oss-cn-shanghai.aliyuncs.com/release/easynlp_modelzoo/"
)
...
def parse_row_by_schema(row: str, input_schema: str) -> dict:
    row_dict = dict()
    for schema, content in zip(input_schema.split(","), row.strip("\n").split("\t")):
...
def get_pretrain_model_path(pretrained_model_name_or_path, disable_auto_download=False):
    if (
        ...
        or pretrained_model_name_or_path.strip() == ""
    ):
        return None

    if (
        pretrained_model_name_or_path.startswith("./")
        or pretrained_model_name_or_path.startswith("/")
        or pretrained_model_name_or_path.startswith("oss://")
    ):
    ...
    if pretrained_model_name_or_path in model_name_mapping:
        ...
        if "oss://" in modelzoo_base_dir:
            ...
            assert io.exists(
                os.path.join(get_dir_name(pretrained_model_name_or_path), "config.json")
        ...
        error_msg = (
            "`%s` is not a existing pre-defined model name. Here're the list: \n"
            % pretrained_model_name_or_path
        )
...
def parse_tf_config():
    ...
    if tf_config_str is None:
        ...
        if (
            "cluster" not in tf_config
            or "task" not in tf_config
            or "type" not in tf_config["task"]
            or "index" not in tf_config["task"]
        ):
            return None, None, None
...
def get_cnn_vocab(args):
    ...
    if os.path.exists(os.path.join(args.checkpoint_dir, "vocab.txt")):
        ...
        return os.path.join(
            ...
            get_pretrain_model_path("bert-small-uncased", disable_auto_download=True),
            "vocab.txt",
        )
    elif args.pretrained_model_name_or_path.endswith("zh"):
        return os.path.join(
            os.path.join(os.environ["HOME"], ".easynlp", "modelzoo"),
            get_pretrain_model_path("bert-small-chinese", disable_auto_download=True),
            "vocab.txt",
        )
    else:
